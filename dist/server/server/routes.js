...
import { generateBatchQuizQuestions, answerDoubtQuery } from "./openai.js";
import { renderDiagram } from "./diagramRenderer.js";
import bcrypt from "bcryptjs";
import session from "express-session";
import { loginSchema, signupSchema, insertDoubtQuerySchema } from "../shared/schema.js";
...
const calculateSpacedRepetitionDates = (startDate) => {
    const dates = [];
...
export async function registerRoutes(app) {
    ...
    app.get("/api/subjects", async (req, res) => {
        try {
            ...
            let subjects;
            if (board && grade) {
                subjects = await storage.getSubjectsByBoardAndGrade(board, parseInt(grade));
            }
            else if (board) {
                subjects = await storage.getSubjectsByBoard(board);
            }
            else if (grade) {
                subjects = await storage.getSubjectsByGrade(parseInt(grade));
            }
            else {
    ...
    app.post("/api/quizzes", isAuthenticated, async (req, res) => {
        try {
            ...
            const userSubjectQuizzes = activeQuizzes.filter(quiz => quiz.subjectId === validatedData.subjectId && quiz.userId === user.id);
            ...
            if (existingQuiz && existingQuiz.quizSets.length === 8) {
                ...
                batchQuestions = {
                    questions: existingQuiz.quizSets.flatMap((set) => set.questions.map((q) => ({
                        ...q,
                ...
                batchQuestions = await generateBatchQuizQuestions(selectedSubject.name, selectedChapter.name, selectedTopic.name, user.grade || 10, user.board || "CBSE", validatedData.questionTypes, validatedData.bloomTaxonomy, validatedData.difficultyLevels, validatedData.numberOfQuestions, validatedData.diagramSupport);
            }
            ...
            batchQuestions.questions.forEach((question) => {
            ...
            for (let setNumber = 1; setNumber <= 8; setNumber++) {
                ...
                const processedQuestions = await Promise.all(setQuestions.map(async (question, index) => {
    ...
    app.get("/api/quizzes/performance", isAuthenticated, async (req, res) => {
        try {
            ...
            const performanceData = await storage.getQuizPerformance(userId, subjectIdNumber, startDateObj, endDateObj);
    ...
    app.post("/api/quizzes/complete/:quizId/:quizSetId", isAuthenticated, async (req, res) => {
        try {
            ...
            const schedules = await storage.getQuizSchedulesByQuizAndSet(parseInt(quizId), parseInt(quizSetId), userId);
            ...
            const allSchedules = await storage.getQuizSchedulesByQuizAndSet(parseInt(quizId), parseInt(quizSetId), userId);
    ...
    app.get("/api/quiz-schedule/:scheduleId/adaptive-questions", isAuthenticated, async (req, res) => {
        try {
            ...
            const quizHistory = userSchedules.filter(s => s.quizId === schedule.quizId &&
                s.status === "completed" &&
                s.score !== null);
            let selectedQuestions = [];
            const allQuestions = Array.isArray(quizSet.questions) ? quizSet.questions : [];
    ...
    app.get("/api/quizzes/today", isAuthenticated, async (req, res) => {
        try {
            ...
            const enrichedSchedules = await Promise.all(schedules
    ...
    app.get("/api/quizzes/completed", isAuthenticated, async (req, res) => {
        try {
            ...
            const enrichedSchedules = await Promise.all(completedSchedules
    ...
    app.get("/api/quizzes/upcoming", isAuthenticated, async (req, res) => {
        try {
            ...
            const enrichedSchedules = await Promise.all(schedules.filter(schedule => schedule.status === "pending").map(async (schedule) => {
                const quiz = await storage.getQuizById(schedule.quizId);
                const quizSet = await storage.getQuizSetById(schedule.quizSetId);
                return {
                    ...schedule,
    ...
    app.post("/api/doubt-queries", isAuthenticated, async (req, res) => {
        try {
            ...
            const todayQueries = doubtQueries.filter(query => query.createdAt && query.createdAt >= today && query.createdAt < tomorrow);
            ...
            try {
                ...
                const matchingSubject = subjects.find(s => s.name.toLowerCase() === validatedData.subject.toLowerCase());
                if (matchingSubject) {
            ...
            const answer = await answerDoubtQuery(questionText, validatedData.subject, parseInt(validatedData.class) || 10, validatedData.board || "CBSE");
    ...
    app.get("/api/quiz-feedback/:quizId", isAuthenticated, async (req, res) => {
        try {
            ...
            const existingFeedback = userFeedback.find(f => f.type === "quiz_feedback" && f.feedbackText?.includes(`Quiz ID: ${quizId}`));
    ...
    app.get("/api/admin/subjects", isAdminAuthenticated, async (req, res) => {
        try {
            const subjects = await storage.getAllSubjects();
            res.json(subjects);
        }
