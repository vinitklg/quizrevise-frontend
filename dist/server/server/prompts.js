...
export function getPromptForSubject(subject) {
    // Normalize subject name for comparison
    const normalizedSubject = subject.trim();
    // Check if we have a specific prompt for this subject
    if (SUBJECT_SPECIFIC_PROMPTS[normalizedSubject]) {
        return SUBJECT_SPECIFIC_PROMPTS[normalizedSubject];
    }
    // Fall back to general prompt
    return GENERAL_PROMPT;
}
// Function to substitute variables in the prompt
export function substitutePromptVariables(prompt, variables) {
