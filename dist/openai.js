...
import { getPromptForSubject, substitutePromptVariables } from "./prompts.js";
// Initialize OpenAI client
const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
// Generate all quiz questions for spaced repetition in a single API call
export async function generateBatchQuizQuestions(subject, chapter, topic, grade, board, questionTypes, bloomTaxonomyLevels, difficultyLevels, numberOfQuestions, diagramSupport = false) {
    try {
...
export async function generateQuizQuestions(subject, chapter, topic, grade, board, questionTypes, bloomTaxonomyLevels, difficultyLevels, numberOfQuestions, setNumber, diagramSupport = false) {
    try {
        const basePrompt = getPromptForSubject(subject);
        const promptVariables = {
            board,
...
- Set number: ${setNumber}`;
        ...
        if (result.questions && Array.isArray(result.questions)) {
            result.questions = result.questions.map((question, index) => {
                ...
                if (fixedQuestion.questionType === "true-false" &&
                    (!question.options || Object.keys(question.options).length === 0)) {
                    ...
                    fixedQuestion.options = Object.entries(question.options).map(([key, value]) => `${key}. ${value}`);
                }
                else if (Array.isArray(question.options)) {
                    fixedQuestion.options = question.options.map((opt, idx) => {
...
// ✅ New Function
export async function answerDoubtQuery(question, subject, grade, board) {
    try {
        const response = await openai.chat.completions.create({
            model: "gpt-4o",
            messages: [
