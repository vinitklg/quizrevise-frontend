...
var Dashboard = function () {
    var _a = useAuth(), user = _a.user, isUserLoading = _a.isLoading;
    var _b = useQuery({
        queryKey: ["/api/quizzes"],
    }), quizzes = _b.data, isQuizzesLoading = _b.isLoading;
    var _c = useQuery({
        queryKey: ["/api/quizzes/today"],
    }), schedules = _c.data, isSchedulesLoading = _c.isLoading;
    var _d = useQuery({
        queryKey: ["/api/quizzes/performance"],
    }), performanceData = _d.data, isPerformanceLoading = _d.isLoading;
    // Get upcoming quizzes to calculate active quizzes
    var _e = useQuery({
        queryKey: ["/api/quizzes/upcoming"],
    }), upcomingQuizzes = _e.data, isUpcomingLoading = _e.isLoading;
    // Calculate stats
    var totalQuizzes = (quizzes === null || quizzes === void 0 ? void 0 : quizzes.length) || 0;
    ...
    var todayPendingQuizIds = new Set();
    // Add quiz IDs from today's pending schedules only
    schedules === null || schedules === void 0 ? void 0 : schedules.filter(function (s) { return s.status === "pending"; }).forEach(function (s) {
        todayPendingQuizIds.add(s.quizId);
    });
    var activeQuizzes = todayPendingQuizIds.size;
    var completedQuizzes = (quizzes === null || quizzes === void 0 ? void 0 : quizzes.filter(function (q) { return q.status === "completed"; }).length) || 0;
    var totalScheduledToday = (schedules === null || schedules === void 0 ? void 0 : schedules.length) || 0;
    var completedToday = (schedules === null || schedules === void 0 ? void 0 : schedules.filter(function (s) { return s.status === "completed"; }).length) || 0;
    ...
    return (<div className="pt-4 pb-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        {isUserLoading || isQuizzesLoading || isSchedulesLoading || isPerformanceLoading || isUpcomingLoading ? (<div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {[1, 2, 3, 4].map(function (i) { return (<div key={i} className="h-32 bg-gray-200 dark:bg-gray-700 rounded-lg animate-pulse"></div>); })}
          </div>) : (<>
              ...
              <Card className="border-l-4 border-l-purple-500 bg-gradient-to-r from-purple-50 to-white dark:from-purple-950 dark:to-gray-900">
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-purple-800 dark:text-purple-200">
                    {totalScheduledToday ? "".concat(calculateProgress(completedToday, totalScheduledToday), "%") : "0%"}
                  </div>
            ...
            <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
              <div className="lg:col-span-2">
                {performanceData && performanceData.length > 0 ? (<PerformanceChart data={performanceData}/>) : (<Card>
                    <CardHeader>
              ...
              <div className="lg:col-span-1">
                <TodayQuizzes userId={(user === null || user === void 0 ? void 0 : user.id) || 0}/>
              </div>
            </div>
          </>)}
      </div>
